...
def set_bits(reg_data, bitname, data):
	return (reg_data & bitname[0]) | \
			((data << bitname[1]) & bitname[0])
...
class BMM150:
	def __init__(self, i2c):
		...
		print("Got chip address: %s" % self.reg_data[0])
		if self.reg_data[0] != CHIP_ID:
			raise ValueError("But expected ID %s" % CHIP_ID)
	...
	def set_power_control_bit(self, bit):
		...
		# self.i2c_read1(POWER_CONTROL_ADDR)
		# self.reg_data1[0] = set_bits_pos_0(self.reg_data1[0], PWR_CNTRL, bit)
		# self.i2c_write(POWER_CONTROL_ADDR, self.reg_data1)
	...
	def read_trim_registers(self):
		...
		print("trims:", trim_x1y1, trim_xyz_data, trim_xy1xy2)
	...
	def set_odr_xyz_rep(self):
		...
		# reg_data = self.i2c_read(OP_MODE_ADDR)
		# reg_data = set_bits(reg_data, ODR, self.settings[data_rate])
		# self.i2c_write(OP_MODE_ADDR, reg_data)
	...
	def read_mag_data(self):
		...
		raw_x = (reg_data[1]*32) | reg_data[0]

		reg_data[2] = get_bits(reg_data[2], DATA_Y)
		raw_y = (reg_data[3]*32) | reg_data[2]

		reg_data[4] = get_bits(reg_data[4], DATA_Z)
		raw_z = (reg_data[5]*128) | reg_data[4]

		reg_data[6] = get_bits(reg_data[6], DATA_RHALL)
		raw_r = (reg_data[7] << 6) | reg_data[6]
